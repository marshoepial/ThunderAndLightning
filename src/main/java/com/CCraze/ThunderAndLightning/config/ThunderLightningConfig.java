package com.CCraze.ThunderAndLightning.config;
...
import static com.CCraze.ThunderAndLightning.setup.ModVals.*;

public class ThunderLightningConfig {
    ...
    private final ConfigGroup mainGroup = new ConfigGroup("Thunder and Lightning Config", new ConfigGroup[]{lightningAttractorGroup}, 0);


    public ThunderLightningConfig(){
        if (!CONFIGFILE.exists()){
            if (!new File("config/"+ ThunderAndLightning.MODID).exists()) new File("config/"+ ThunderAndLightning.MODID).mkdirs();
